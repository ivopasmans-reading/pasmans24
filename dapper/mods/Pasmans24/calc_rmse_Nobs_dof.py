...
# Directory
fig_dir = os.path.join(exp.fig_dir,'rmse_Nobs_dof')
...
def exp_spectral_dof(T=50, Nens=16, sig_obs=1.0):
    ...
    #DG orders
    orders = np.array([0,1,2,4,6,8,10])
    dof = exp.Ncells * (1+orders[-1])
    ...
    tree.add_model('lin', int(dof / 1), name="Nodal")
    ...
    for order in orders:  
        ...
        tree.add_model('dg', int(dof / (order+1)), order=order,
                       name='DG{:02d}'.format(order))
        
    # Experiment
    tree.add_xp(E3DVar, Nens, loc_rad=exp.L)
