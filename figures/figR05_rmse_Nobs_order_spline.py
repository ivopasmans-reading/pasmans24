...
from dapper.mods.Pasmans24.calc_rmse_Nobs_order_noloc import run, io
from dapper.mods.Pasmans24.calc_rmse_Nobs_order_spline import run as runSpline
from dapper.mods.Pasmans24.calc_rmse_Nobs_order_spline import io as ioSpline
from scipy.stats import bootstrap 
import re

io.try_loading, ioSpline.try_loading = True, True
    ...
    def add_orders(self, xps):        
        for xp in xps:
            ...
            if '_model_type' not in options:
                pass 
            elif options['_model_type'] == 'lin' and hasattr(xp.parent,'_interpolator'):
                self.add_order(xp, xp.parent._interpolator[6:])
...
if __name__ == '__main__':
    tree = run()
    treeSpline = runSpline()

    #truths = exp.collect(tree, 1)
    #models = exp.collect(tree, 3)
    xps = exp.collect(tree, 4) + exp.collect(treeSpline,4)
    #xps = exp.collect(treeSpline,4)

    fig = FigureNobsError('figR05_pasmans_rmse_Nobs_order_spline_slope40.png')
