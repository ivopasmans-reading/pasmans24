...
from dapper.mods.Pasmans24.calc_rmse_Nobs_order_randomObs import run, io, SubEnsemble
...
class FigureNobsError(FigureLines):
    ...
    def add_order(self, xp, label):
        ...
        for ndiff in range(np.size(self.xtrue,0)):
            ...
            if False and label=='Nodal' and Nobs==79 and ndiff==0:
                plt.figure()
                plt.plot(self.r, xtrue1[7], 'k-')
                plt.plot(self.r, xfor1[7], 'r-')
                plt.plot(self.r, xana1[7], 'g--')
                plt.plot(xp.HMM.model.obs_coords(8), xp.yy[7], 'ko')
                print('ERROR ',self.errors[label][Nobs][0,7])
...
if __name__ == '__main__':
    ...
    fig = FigureNobsError('figR02_pasmans_rmse_Nobs_random_s01.png')
    sel = select(fig, xps, criteria={'_slope':[-1.0],'_sig':[1.0]})
    fig.add_orders(sel)
    fig.preplot()
    fig.plot()
    #fig.save()
