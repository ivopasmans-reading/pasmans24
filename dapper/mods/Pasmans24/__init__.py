...
class SpectralModel:
    ...
    def boundary2local(self):
        #0,0,1,1,...
        cells = np.arange(0,self.ncells)
        cells = np.array([cells,cells]).T.flatten()
        #-1,1,-1,1
        local_coords = np.ones_like(cells)
        local_coords[0::2] *= -1
        
        return np.array(local_coords, dtype=float), np.array(cells, dtype=int)
    ...
    def state2legendre(self, x, ndiff=0):
        ...
        def basis(ndiff):
            ...
            return [p.deriv(ndiff) * (2/dx)**ndiff for p in basis]
        
        def interpolator_interior(x,ndiff,local,cell):
            P = np.array([p(2*local-1) for p in basis(ndiff)]).T
            y = np.zeros((np.size(x, 0), len(cell)))
            for n, celln in enumerate(cell):
                y[:, n] += x[:, celln, :]@P[n, :].T
            return y 
        
        def coef_jump(x, ndiff):
            if ndiff==0:
                return np.zeros_like(x)
            
            xjump = coef_jump(x, ndiff-1)
            #Basis functions at boundaries (2,order)
            P = np.array([p([-1.,1.]) for p in basis(ndiff-1)]).T 
            #Function values at cell boundaries (-1,cells,2)
            B = (x+xjump)@P.T
            #Discontinuities at both sides of cell. 
            P0 = np.array([p([-1.,1.]) for p in basis(0)]).T 
            right = 0.5*(np.roll(B[:,:,0],-1,axis=1)-B[:,:,1])[...,None]*P0[1].reshape((1,1,-1))
            left  = 0.5*(np.roll(B[:,:,1], 1,axis=1)-B[:,:,0])[...,None]*P0[0].reshape((1,1,-1))
            #Contribution jump to coefficients. 
            metric = (np.arange(self.order+1) + 0.5).reshape((1,1,-1))
            xjump = (right - left)*metric
                        
            return xjump 

        def interpolator(coords):
            xjump = coef_jump(x, ndiff)
            
            coords = np.reshape(coords, (-1))
            local, cell = self.global2local(coords)
            y = interpolator_interior(x, ndiff, local, cell)
            yjump = interpolator_interior(xjump, 0, local, cell)*(2/dx)**ndiff
            return y+yjump
    ...
    def step(self, x, t, dt):
        ...
        """
        if np.ndim(x) == 1:
            m = np.mod(self.member, len(self.functions))
            x[:] = self.functions[m](self.dyn_coords, t)
        elif np.ndim(x) == 2:
            for n in range(np.size(x, 0)):
                m = np.mod(n + self.member, len(self.functions))
...
class LegendreFunction:
    ...
    def build4integrate(self):
        ...
        self.exp_ind = self.k > 2 * np.pi / (0.5 * self.dx) 
