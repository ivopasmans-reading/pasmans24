...
def taper_factory(radius, tag=None, period=np.inf):
    """
    ...
    def distance(a,b,radius,period):
        ...
        """
        smod = lambda x1,x2: np.mod(x1+.5*x2, x2) - .5*x2 
        ...
        if np.ndim(radius)==0:
            radius = np.ones((1,dim)) * radius
        else:
            radius = np.reshape(radius, (1,-1))
        if np.ndim(period)==0:
            period = np.ones((dim,)) * period
        if np.any(radius<=0.):
            raise ValueError("Radii must be strictly positive.")
        
        #Rescale points into 2D array
        b = np.reshape(b, (-1,dim) )
        a = np.reshape(a, ( 1,dim) ) 
        
        #Apply period if specified.
        d = (a - b).T 
        d = np.array([smod(d1,period1) for d1,period1 in zip(d,period)])
        
        #Calculate distance
        d = np.linalg.norm(d.T/radius, axis=1)
        return d
    ...
    if tag == 'Gauss':
        def taper(a,b):
            return np.exp(-0.5 * distance(a,b,radius,period)**2)
    elif tag == 'Exp':
        def taper(a,b):
            return np.exp(-0.5 * distance(a,b,radius,period)**3)
    elif tag == 'Cubic':
        R = radius * 1.87  # Sakov: 1.8676
        def taper(a,b):
            d = distance(a, b, R, period)
            return np.where(d<=1, (1-d**3)**3, 0.)
    elif tag == 'Step':
        def taper(a,b):
            d = distance(a, b, radius, period)
...
class LegendreTaperer(Taperer):   
    ...
    def update_taper(self, radius, time, tag):
        ...
        #Check shape radius
        if np.ndim(radius)==0:
            radius = np.ones((self.order+1,1)) * radius
        ...
        def taper(a,b):
            ...
            taper_template = taper_factory(radius[order], tag[order], period=self.period)
            ...
            if np.size(b,-1)==np.size(a,-1):           
                ...
                orders = np.array(b[:,-1], dtype=int)==order
                tapering = np.zeros_like(orders)
                tapering[orders] = taper_template(a, b[orders,:-1])
            else:
