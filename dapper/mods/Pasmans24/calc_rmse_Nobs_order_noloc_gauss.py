...
# Directory
fig_dir = os.path.join(exp.fig_dir,'rmse_Nobs_order_noloc_gauss')
...
def exp_spectral_Nobs(T=50, Nens=16, sig_obs=1.0):
    ...
    # Truth    
    #for length in np.array([.25,.5,.75,1.,1.5,2,3,4,5,6,10,15,20,25,40]) * exp.L / exp.Ncells:
    for length in np.array([.5,1,10]) * exp.L / exp.Ncells:    
        tree.add_truth(length=length, factory='gauss',
                       name='scale={:d}'.format(int(length*1e-3)))
    
    # Observation
    for sig in np.array([1.0]): #np.array([0.5,1.0,1.5]):
        Nobs_list  = [int(exp.Ncells * o) for o in [.1,.5,1,1.5,3,5,9,13]]
        #Nobs_list = [int(exp.Ncells*o) for o in [1,3,9]]
