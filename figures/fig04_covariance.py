...
class FigureCovariance(FigureLines):
    ...
    def preplot(self):
        self.create_panels((3,2), loc=(-.17,.96))
        self.fig.set_size_inches((portrait['width'],portrait['height']),
                                 forward=True)
        self.fig.subplots_adjust(right=.99, left=.08, top=.99, bottom=.12,
                                 wspace=.2,hspace=.22)
    ...
    def add_colorbar(self):
        color, vlims, norm = self.color_setting()
        cax = self.fig.add_axes((0.14, .03, .8, .02))
...
def plt_figure(xps, N, fig_name):
    ...
    fig = FigureCovariance(fig_name+"N{:02d}".format(N)+'.png')
    fig.preplot()
    ...
    print('cov',np.min(cov),np.max(cov))
    fig.add_cov_state(fig.axes[4], cov)
    
    #dg
    E = xps[1].E['for'][-1]
    cov = calc_cov(E, order)
    print('cov',np.min(cov),np.max(cov))
    fig.add_cov_dg(fig.axes[1], cov)
    
    cov = calc_cov(E[:N], order)
    print('cov',np.min(cov),np.max(cov))
...
if __name__ == '__main__':
    ...
    cov, fig = plt_figure(xps, 16, 'fig09_pasmans_covariance')
