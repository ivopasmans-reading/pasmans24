...
class E3DVar:
    ...
    def assimilate(self, HMM, xx, yy):
        ...
        for k, ko, t, dt in progbar(HMM.tseq.ticker, disable=True):
            inflation = ((self.N+1)/(self.N-1))**0.5
            
            # Move ensemble forward in time.
            E  = HMM.Dyn(E, t-dt, dt) 
            dE = HMM.Dyn.noise.sample(self.N)
            E += dE
    ...
    def create_B(self, HMM, time, A):
        ...
        taperer = HMM.Obs.localization(self.loc_rad, 'x2x', time, self.taper)
    ...
    def create_solver(self, Bhat):
        ...
        """
        from dapper.tools.solvers import RBCG, RCG, IterationsChecker, DirectSolver
        ...
        checkers = [IterationsChecker(20), 
                    RelativeSolutionChecker(1e-2, norm=norm),
                    RelativeResidualChecker(1e-3),
