...
# Directory
fig_dir = os.path.join(exp.fig_dir,'rmse_Nobs_order_randomObs')
...
def exp_spectral_randomNobs(T=50, Nens=16, sig_obs=1.0):
    ...
    for sig in np.array([1.0]):
        Nobs_list  = [int(exp.Ncells * o) for o in [.05,.1,.2,.5,1,1.5,3,5,9,13]]
        #Nobs_list  = [int(exp.Ncells * o) for o in [1.]]
        for Nobs in Nobs_list:
            tree.add_obs(Nobs, sig, obs_type='landmask',
    ...
    # Experiment
    tree.add_xp(E3DVar, Nens, loc_rad=None)
